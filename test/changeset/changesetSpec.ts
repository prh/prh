...
describe("ChangeSet", () => {
    describe(".constructor", () => {
        it("can reject Overlapping diff, pattern p.index === c.index", () => {
            ...
            expect(changeSets.diffs.length).toBe(1);
            expect(changeSets.diffs[0].matches[0]).toBe("例えば");
        });
        it("can reject Overlapping diff, pattern p.index !== c.index", () => {
            const base = "ABCDE";
            let diffs: Diff[] = [];
            ...
            const changeSets = new ChangeSet({ content: base, diffs });
            expect(changeSets.diffs.length).toBe(1);
            expect(changeSets.diffs[0].matches[0]).toBe("AB");
        });
    });
    describe(".applyChangeSets", () => {
        it("can apply change sets. pattern shorten", () => {
            ...
            expect(result).toBe("今日の晩御飯はWebさ！そしておかずもWebさ！");
        });
        it("can apply change sets. pattern longer", () => {
            ...
            const changeSets = new ChangeSet({ content: base, diffs });
            const result = changeSets.applyChangeSets(base);
            expect(result).toBe("今日の晩御飯はウェッブだ！そしておかずもウェッブだ！");
        });
        it("can apply change sets with grouping", () => {
            ...
            const changeSets = new ChangeSet({ content: base, diffs });
            const result = changeSets.applyChangeSets(base);
            expect(result).toBe("白いご飯と伯方(博多ではない)の塩(潮ではない)！");
        });
        it("can apply change sets with grouping", () => {
            ...
            const result = changeSets.applyChangeSets(base);
            expect(result).toBe("ある日のWebと、とある日のWebの話");
        });
    });
    describe(".makeChangeSet", () => {
        it("make change sets", () => {
            const base = "今日はjs明日はts明後日はなんのaltjsですかねぇ？";
            const changeSets = makeChangeSet("tmp.txt", base, /JS/gi, "JS");

            const result = changeSets.applyChangeSets(base);
            expect(result).toBe("今日はJS明日はts明後日はなんのaltJSですかねぇ？");
        });
    });
    describe(".subtract", () => {
        it("subtract changeset from other changeset", () => {
            const base = "次に見るjs（@<list>{hoge-js}）はただのjsではありません。\nしかし、altjs（@<fn>{alt-js}）ほどではないでしょう。";
            const minuend = makeChangeSet("tmp.txt", base, /js/gi, "JS");
            const subtrahend = makeChangeSet("tmp.txt", base, /(@<(?:list|fn)>{.+?})/gi, "$1");

            const changeSets = minuend.subtract(subtrahend);

            const result = changeSets.applyChangeSets(base);
            expect(result).toBe(
                "次に見るJS（@<list>{hoge-js}）はただのJSではありません。\nしかし、altJS（@<fn>{alt-js}）ほどではないでしょう。",
            );
        });
        it("not subtract changeset with over wrapped changeset", () => {
            const base = "js(@<list>{js})js";
            const minuendA = makeChangeSet("tmp.txt", base, /\((.+?)\)/gi, "（$1）");
            const minuendB = makeChangeSet("tmp.txt", base, /JS/gi, "JS");
